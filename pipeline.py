...
def run_pipeline(concept: dict):
    ...
    try:
        ...
        if cfg.instagram_enabled and cfg.instagram_access_token and cfg.instagram_user_id:
            log.info("Step 9: [Instagram 게시] 썸네일 이미지 게시 시작...")
            ...
            instagram_result = ig_uploader.post(thumbnail, caption)
            ...
            if instagram_result:
                ...
                log.warning("Instagram 게시 실패 — YouTube 업로드는 유지됨")
        else:
            log.info("Step 9: [Instagram 게시] INSTAGRAM_ENABLED=false 또는 토큰 미설정 — 스킵")
