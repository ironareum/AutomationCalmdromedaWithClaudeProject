...
사운드 레이어링 + 영상 루프 합성 → 1~3시간 유튜브 영상 생성
"""
...
class VideoProducer:
    ...
    def _check_ffmpeg(self):
        try:
            ...
            raise RuntimeError("FFmpeg not found. Install: brew install ffmpeg / apt install ffmpeg")
    ...
    def mix_sounds(self, sound_files: list[Path], target_duration: int) -> Path | None:
        ...
        if len(layers) == 1:
            ...
            cmd = [
                ...
                "-af", "afade=t=out:st={},d=5".format(target_duration - 5),  # 마지막 5초 페이드아웃
                "-b:a", "192k",
                str(output)
            ]
        else:
    ...
    def prepare_video_loop(self, video_files: list[Path], target_duration: int) -> Path | None:
        ...
        repeat_times = max(1, int(target_duration / total_clip_duration) + 2)
    ...
    def merge(self, video_loop: Path, audio: Path, output_path: Path) -> Path | None:
        ...
        if self._run(cmd, "Merging video + audio → final MP4"):
